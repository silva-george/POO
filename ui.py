...
#Menus
    ...
    def menu_cliente():
        print("1 - Listar Produtos | 2 - Adicionar Produto no Carrinho \n3 - Fechar Pedido | 4 - Ver Meus Pedidos")
        print("0 - Sair | 99 - Fim")
        op = int(input("\nInforme uma opção: "))
        if op == 0: UI.sair_do_sistema()
...
#Vendas       
    ...
    def cliente_adicionar_produto(cls):
        ...
        for venda in View.venda_listar():
            if venda.id_cliente == cls.cliente_id and venda.carrinho == True:
                venda_atual = venda
        View.inserir_produto_venda_item(0, qtd, produto.preco, venda_atual.id, produto.id)
        View.atualizar_total_venda(venda_atual, (qtd * produto.preco))
        
        print(f"\nProduto adicionado ao carrinho: ")
        print(f"{qtd}x - {produto.descricao},  - R$ {produto.preco:.2f} - Total: R$, {produto.preco * qtd:.2f}")
        print(f"\nTotal do carrinho: {venda_atual.total + (qtd * produto.preco):.2f}")
        print("\n")
    ...
    def cliente_fechar_pedido(cls):
        ...
        for venda in View.venda_listar():
            if venda.id_cliente == cls.cliente_id and venda.carrinho == True:
                venda_atual = venda
        print("\nID da compra: ", venda_atual.id)
        for item in View.venda_item_listar():
            if item.id_venda == venda_atual.id:
                print(f"{item.qtd}x - ID produto: {item.id_produto} - R$ {item.preco:.2f} - Total: R$, {item.preco * item.qtd:.2f}")
                total = total + (item.preco * item.qtd)
        print(f"\nValor total do carrinho: R${venda_atual.total:.2f}")
        op_fechar = int(input("Para fechar o pedido digite: 1 - Para continuar comprando : 0 \n"))
        if op_fechar == 1:
            View.fechar_venda(venda_atual)
            View.criar_venda(0, datetime.datetime.now().strftime("%d/%m/%Y"), True, 0, venda.id_cliente)
            print("\nPedido fechado com sucesso, você pode vê-lo abaixo:")
            UI.cliente_meus_pedidos()
            print("\nSeu novo carrinho ja estaá pronto para compra")
        if op_fechar == 0:
            print("Boas compras!")
            print("\n")
    ...
    def cliente_meus_pedidos(cls):
        print("\nMeus Pedidos\n")
        for venda in View.venda_listar():
            if venda.id_cliente == cls.cliente_id and venda.carrinho == False:
                print("Pedido fechado: ",venda)
        for venda in View.venda_listar():
            if venda.id_cliente == cls.cliente_id and venda.carrinho == True:
                print("\nPedoido em aberto: \n",venda, "\n")
                venda_atual = venda
